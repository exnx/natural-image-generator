...
def random_back_color():
    color = list(np.random.choice(range(20, 150), size=3))
    color = tuple(color)  # convert to tuple
    return color

def random_font_color():
    color = list(np.random.choice(range(130,230), size=3))
    color = tuple(color)  # convert to tuple
    return color
    ...
    for font_idx, font_name in enumerate(font_names_list):
        ...
        for char in chars:
            ...
            for i in range(N):
                ...
                draw_object.text((width_pos, height_pos), char, align='center', font=font_obj, fill=random_font_color())
